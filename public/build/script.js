...
let ready = JSON.parse(localStorage.getItem("ready")) && true;
...
async function fetchData(mode, pokemon = "") {
  try {
    ...
    if (
      !prevSearchIds.includes(data.id) ||
      mode === "currLoad" ||
      mode === "prevSearch" ||
      mode === "typedSearch"
    ) {
      ...
      if (!ready) {
        ...
        if (!prevEntry || mode === 'prevSearch') {
          ...
          document
            ...
            .insertAdjacentHTML("afterend", dropItemStr);
          const currSearch = {};
          currSearch[data.id] = dropItemStr;

          prevSearches = { ...currSearch, ...prevSearches };
          if (!prevSearchIds.includes(data.id)) {
...
function addNothingMsg() {
  previousSearchMenu.innerHTML +=
    '<p class="pointer-events-none nothing-msg text-center p-3">Nothing...</p>';
}
